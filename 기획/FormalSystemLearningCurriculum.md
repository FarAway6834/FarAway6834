# lan'molan' based mathmatical logic curriculum & AlkalicLava System with ezeronedoperodepredrefunctionalang based Formal System Learning Curriculum

내용
 - Formal System Learning Curriculum : 형식문법이론, 형식언어, 모델론적 언어, 이론전산/이론컴퓨터과학
 - Alkalic증명 체계와 ezeronedoperodepredrefunctionalang의 함수 체계, Lava의 추상대수 채계를 이용한 수학 기초론 AlkalicLava System with ezeronedoperodepredrefunctionalang : AlkalicLava, ezeronedoperodepredrefunctionalang, ZFC, 방정식을 통한 수리논리, 추상대수와 모델론
 - lan'molan' based mathmatical logic curriculum : 라틴어 일부, 고대 그리스어 일부, lan'molan', 명제논리, 술어논리, 정언논리, 아리스토텔레스 범주론, 집합론

## 기대 독자 : Python&Cython&C/C++가 홈 랭귀지고, Java/C# + HTML/CSS/JS + Haskell + Lambda Calculus를 다뤄본 적 있는 개발자인데, ezeronedoperodepredrefunctionalang를 배워서 수학에 대해 깊이있게 이해하고자 하는 자. 물론 lan'molan'은 구성 원리가 아니라, 문법 번역식 교수법 & 예문을 통한 생습기제 학습로, 일반인용으로 배움.

한마디로 내 사고 궤적을 따라가고자 하는 사람에게 선물할 무료 책이다.

## 학습 커리큘럼

1. ezeronedoperodepredrefunctionalang의 문법과 사용방법을, 프로그래밍 언어 배우듯 설명한 교재를 준비한다. 그걸로 ezeronedoperodepredrefunctionalang를 접한다
2. ezeronedoperodepredrefunctionalang를 통해 해설된 Alkalic & Laval System과 추상대수를 배운다
3. 대수적인 명제논리 및 라틴어&고대 그리스어 기반 오르가논(οργανον, 아리스토텔레스 범주론, 정언논리, Proto-Indo-European적 OMNES감각)를 배운다. 물론 라틴어와 그리스어는 듀오링고/어원공부/성경의 예문/lan'molan'을 통한 해설을 교재와 곁들여서 필수적인 부분만 알려준다.
4. 집합론적인 술어논리와 집합론을 배운다.
5. 추상대수를 통하여 이론전산학과 형식문법이론을 배운다.
6. ezeronedoperodepredrefunctionalang으로 해설된 모델론을 배운다.
7. ezeronedoperodepredrefunctionalang가 어케 설계되었는지 표준 모델론을 기반으로 배운다.

## 교재 언어 (교재에 쓰일 자연어) : lan'molan'
...
## 교재의 수학기초언어체계 : ezeronedoperodepredrefunctionalange

ezeronedoperodepredrefunctionalang는 ezerone타입이 도입된 operodepredrefunctionalang이므로, 프로그래밍언어같은 구문과 직관으로 모델론을 작성하는것이다.
...
## 증명이론 등 실질적인 수학기초론을 맡을 이론 : Alkalic + Lava
